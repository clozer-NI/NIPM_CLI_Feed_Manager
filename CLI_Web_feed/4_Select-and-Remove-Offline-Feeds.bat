...
if /i "%FEED_SELECTION%"=="all" (
    ...
    REM Use PowerShell to properly parse feed names from the output
    for /f "usebackq delims=" %%F in (`powershell -NoProfile -Command "Get-Content '%TEMP_FEED_LIST%' | Where-Object { $_ -notmatch 'https' } | ForEach-Object { ($_ -split '\\s+')[0] } | Where-Object { $_ -ne '' }"`) do (
        set /A TOTAL+=1
